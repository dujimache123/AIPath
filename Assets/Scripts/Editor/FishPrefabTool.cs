...
public class FishPrefabTool : EditorWindow
{
	static public FishPrefabTool instance;
	...
	public void Reset ()
	{
		...
		if (mTab == 0)
		{
			...
			{
				if (s.EndsWith(".prefab") && s.Contains("Prefab_Fish_"))
					filtered.Add(s);
			}
	...
	string saveKey { get { return "Fish " + Application.dataPath + " " + mTab; } }
	...
	void Load ()
	{
		mTab = NGUISettings.GetInt("Fish Prefab Tab", 0);
		mMode = NGUISettings.GetEnum<Mode>("Fish Prefab Mode", mMode);
	...
	void GeneratePreview (Item item, UISnapshotPoint point)
	{
...
#endif
		{
			...
			string path = "Assets/Scripts/Editor/Preview/" + item.prefab.name + ".png";
#if UNITY_4_3 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7
	...
	static void SetupSnapshotCamera (GameObject go, Camera cam, float objectSize, float orthoSize, float near, float far)
	{
		...
		Transform snapshot = FindChild(go.transform, "Fish Snapshot Point");
		...
		string str = snapshot.name.Replace("Fish Snapshot Point", "");
	...
	void OnGUI ()
	{
		...
		if (mTab != newTab)
		{
			...
			mTab = newTab;
			mReset = true;
			NGUISettings.SetInt("Fish Prefab Tab", mTab);
			Load();
		}
			...
			for (int i = 0; i < indices.size; ++i)
			{
				...
				if (GUI.Button(rect, mContent, "Button"))
				{
					...
					else if (currentEvent.button == 1)
					{
						...
						NGUIContextMenu.AddItem("Add To Season Info", false, AddToSeasonInfo, index);
						NGUIContextMenu.Show();
					}
				}
		...
		if (modeAfter != mMode)
		{
			mMode = modeAfter;
			mReset = true;
			NGUISettings.SetEnum("Fish Prefab Mode", mMode);
		}
	}

	void AddToSeasonInfo(object obj)
	{
		if (this == null ) return;
        if (Selection.activeTransform == null || Selection.activeTransform.GetComponent<FishSeasonInfoComponent>() == null)
        {
            EditorUtility.DisplayDialog("error", "错误的节点", "OK");
            return;
        }
		int index = (int)obj;
		if (index < mItems.size && index > -1)
		{
			Item item = mItems[index];
			GameObject fish = (GameObject)Instantiate(item.prefab);
			fish.transform.parent = Selection.activeTransform;
			fish.transform.localPosition = Vector3.zero;
			fish.transform.localScale = item.prefab.transform.localScale;
			fish.transform.eulerAngles = Vector3.zero;
            fish.name = item.prefab.name;
		}
	}
}
